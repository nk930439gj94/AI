...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            if self.beliefs[p] > 0:
                newDistr = self.getPositionDistribution( self.setGhostPosition(gameState,p) )
                for newPos, newprob in newDistr.items():
                    allPossible[newPos] += ( self.beliefs[p] * newprob )
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self._particles.append( self.legalPositions[ i%n ] )
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance == None:
            self._particles = [ self.getJailPosition() for i in range(self.numParticles) ]
            return
        ...
        self._particles = util.nSample( distribution, values, self.numParticles )
    ...
    def elapseTime(self, gameState):
        ...
        for p in self._particles:
            newDistr = self.getPositionDistribution(self.setGhostPosition( gameState, p ))
            newparticles.append( util.sample(newDistr) )
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        all = list( itertools.product(self.legalPositions,repeat=self.numGhosts) )
        random.shuffle(all)
        self.particles = []
        n = len(all)
        for i in range(self.numParticles):
            self.particles.append( all[i%n] )
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        inJail = [ i==None for i in noisyDistances ]
        allPossible = util.Counter()
